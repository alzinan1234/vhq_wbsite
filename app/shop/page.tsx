...
export default function ShopPage() {
  const { isLoggedIn, merchProducts, merchLoading, loadMerchProducts, cart, addToCart, removeFromCart, clearCart, showToast } = useStore();
      ...
      <Modal open={cartOpen} onClose={() => setCartOpen(false)} title={`Cart (${cartCount})`}>
        {cart.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            <div className="text-xs text-center" style={{ color:"var(--tx3)" }}>Payment processed via RevenueCat in the mobile app</div>
            <div className="flex gap-3">
              <button className="btn btn-ghost btn-sm flex-1" onClick={clearCart}>Clear Cart</button>
              <button className="btn btn-pk btn-md flex-1" onClick={() => { showToast("Purchase via VHQ mobile app!"); setCartOpen(false); }}>
                Checkout
              </button>
            </div>
          </div>
